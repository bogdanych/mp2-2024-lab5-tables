...
private:
    ...
    void increase_size() {
        // не проводим логирование, т.к. не подразумевается по условию
...
public:
    ...
    void insert(const std::string& key, const Polynom& pol) {
        // не менее половины элементов могут быть пустыми
        // жертвуем памятью, чтобы не деградировать в линию (странно звучит)
        oper++;
    ...
    void remove(const std::string& key) {
        ...
        int i;
        // Гарантированно переберем все элементы таблицы, если не учитывать выхода из цикла
        ...
        // если прошли цикл и не нашли - жалеем о потраченом времени
        oper++;
        if (i < size) {
            currentSize--;
            is_del[index] = 1;
    ...
    Polynom& operator[] (const std::string& key)
    {
        ...
        throw std::exception("Полином не найден");
    }
